...
export class SovereignRegistryParser extends IRegistryParser {
    ...
    getOrderedPaths(registryPath: string): string[] {
        if (!existsSync(registryPath)) {
            throw new Error(
                `CRITICAL: Sovereign Registry not found at ${registryPath}. Check your .atl directory.`
            );
        }
        ...
        if (sectionIndex === -1) {
            throw new Error(
                "MALFORMED REGISTRY: Missing '## User Skills' section. Architectural sovereignty is at risk!"
            );
        }

        // 2. Mandatory Table Validation
        const tableHeaderIndex = lines.findIndex(
            (l, i) => i > sectionIndex && l.includes("| Trigger | Skill | Path |")
        );
        if (tableHeaderIndex === -1) {
            throw new Error(
                "MALFORMED REGISTRY: Missing skills table under '## User Skills'. Any IA touching this must be punished."
            );
        }
        ...
        if (orderedPaths.length === 0) {
            throw new Error(
                "EMPTY REGISTRY: No skills found in the table. Documentation bundle would be incomplete."
            );
        }
