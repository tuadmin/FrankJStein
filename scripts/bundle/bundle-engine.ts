...
import { IBundleService, IRegistryParser, BUNDLE_CONFIG, IScriptLogger } from "./bundle.types.ts";
...
export class BundleEngine extends IBundleService {
    ...
    private fullPayload: string = `# FrankJStein: Sovereign AI Context\n\n> Comprehensive architectural patterns and rules for AI Agents.\n\n`;
    private navigationIndex: string = `# FrankJStein AI Index\n\n> Global roadmap for LLMs and Agentic AI.\n\n`;

    /** @inheritdoc */
    async run(): Promise<void> {
        this.logger.info("Initiating Sovereign AI Documentation Orchestration...");

        const orderedPaths = this.parser.getOrderedPaths(join(BUNDLE_CONFIG.ROOT, BUNDLE_CONFIG.REGISTRY_PATH));
        
        this.logger.info(`Sovereign sequence detected (${orderedPaths.length} skills). Building context...`);
    ...
    private processSkills(orderedPaths: string[]): void {
        ...
        if (orphans.length > 0) {
            this.logger.error(`INTEGRITY VIOLATION: Discovered ${orphans.length} skills not present in the registry.`);
            orphans.forEach(o => this.logger.error(`Missing from Registry: ${relative(BUNDLE_CONFIG.ROOT, o)}`));
            throw new Error("Registry Integrity Failure: All discovered skills must be explicitly registered in .atl/skill-registry.md");
        }
    }
    ...
    private finalize(): void {
        ...
        console.log(`  - Payload: ${BUNDLE_CONFIG.OUTPUT_FULL} (${(this.fullPayload.length / 1024).toFixed(2)} KB)`);
    }
}
