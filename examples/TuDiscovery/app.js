...
const app = new TuJsHtml(function (tags) {
    const { div, h1, p, "button[style=padding: 10px; cursor: pointer; margin-right: 10px;]": button, pre, hr } = tags;
    ...
    div({ "@attrs": { style: "max-width: 600px; margin: auto; padding: 20px;" } }, () => {
        ...
        button({
            style: { background: "#007bff", color: "white", border: "none", borderRadius: "4px;" },
            onclick: async () => {
                loading.value = true;
        ...
        p`Salida:`;
        pre({
            "@attrs": { style: "background: #eee; padding: 10px; border-radius: 4px; min-height: 50px; white-space: pre-wrap;" }
        }, output);
